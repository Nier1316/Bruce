 ...
 * ============================================================ */
static uint8_t  s_rx_staging[UART_BRIDGE_FRAME_LEN]; /* 暂存一帧完整数据 */
static volatile uint8_t s_new_cmd = 0U;              /* 1 = 有待处理的新指令 */
static Uart_instance_t *s_uart_inst = NULL;

static void uart_bridge_rx_callback(void)
{
    if (!s_uart_inst) return;

    /* 只接受长度完全匹配的帧，丢弃其他 */
    if (s_uart_inst->rx_data_len == UART_BRIDGE_FRAME_LEN)
    {
        memcpy(s_rx_staging, s_uart_inst->rx_buffer, UART_BRIDGE_FRAME_LEN);
        s_new_cmd = 1U;
    }
}
...
{
    memset(tx, 0x00U, UART_BRIDGE_FRAME_LEN);
    tx[0]  = UART_BRIDGE_HDR_FB;
    tx[1]  = fb->id;
...
void Uart_motor_bridge_task(void const *argument)
{
    ...
    static uint8_t tx_buf[UART_BRIDGE_FRAME_LEN];
    static uint8_t rx_local[UART_BRIDGE_FRAME_LEN]; /* 任务内局部副本，避免处理中被覆盖 */

    /* --- 初始化串口 (huart3，连接上位机) --- */
    s_uart_inst = Uart_register(&huart3, uart_bridge_rx_callback);

    /* --- 初始化电机，切换至阻抗模式 --- */
    Ele_motor_init(UART_BRIDGE_MOTOR_ID);
    Ele_motor_write_control_mode(UART_BRIDGE_MOTOR_ID, ELE_MOTOR_MODE_IMPEDANCE);

    Ele_motor_feedback_t fb = {.id = UART_BRIDGE_MOTOR_ID};
    ...
    for (;;)
    {
        ...
        if (s_new_cmd)
        {
            ...
            taskENTER_CRITICAL();
            memcpy(rx_local, s_rx_staging, UART_BRIDGE_FRAME_LEN);
            s_new_cmd = 0U;
            taskEXIT_CRITICAL();
            ...
            if (unpack_cmd_frame(rx_local, &pos_ref, &vel_ref, &kp, &kd, &torque_ff, &flags))
            {
                if (flags & 0x01U)  /* bit0 = 1：使能发送 */
                {
                    Ele_motor_set_para(pos_ref, vel_ref, kp, kd, torque_ff,
                                       ELE_MOTOR_MODE_IMPEDANCE, UART_BRIDGE_MOTOR_ID);
                }
            }
        }
        ...
        Uart_sendData(s_uart_inst, tx_buf, UART_BRIDGE_FRAME_LEN);

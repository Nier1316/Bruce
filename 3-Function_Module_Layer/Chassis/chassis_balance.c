...
void ChassisInit(void)
{
    ...
    MotorInit(&CHASSIS.wheel_motor[0], 1, WHEEL_CAN, DJI_M3508, W0_DIRECTION, 1, 0);
    MotorInit(&CHASSIS.wheel_motor[1], 2, WHEEL_CAN, DJI_M3508, W1_DIRECTION, 1, 0);
...
{
    // 1. 计算 dt (微分时间)
    // 所有的速度(v=dx/dt)和加速度(a=dv/dt)计算都依赖这个 duration。
    CHASSIS.duration = xTaskGetTickCount() - CHASSIS.last_time;
    CHASSIS.last_time = xTaskGetTickCount();

    // 2. 依次调用子观测器
    UpdateMotorStatus();     // 读电机（最基础）
    UpdateLegStatus();       // 算腿（正运动学 + 动力学）
    UpdateBodyStatus();      // 算身体（IMU 坐标变换）
    UpdateCalibrateStatus(); // 看校准状态
    UpdateStepStatus();      // 看跳跃状态

    // 3. 数据融合
    BodyMotionObserve(); // 卡尔曼滤波算速度    
 ...
 * @param  none
 */
 // 算腿(UpdateLegStatus) —— 正运动学与力反馈
 // 几何：关节->虚拟腿(知道腿长和角度）
 // 力：关节力矩->地面支持力(知道有没有离地)。
static void UpdateLegStatus(void)
{
    uint8_t i = 0;
    // =====更新关节姿态=====
    ...
    for (i = 0; i < 2; i++) {
        ...
        // 更新位置信息
        // 2. 正运动学解算 (chassis_balance_extras.c)
        // 输入：两个电机角度
        // 输出：虚拟腿长 L0, 虚拟摆角 Phi0
        ...
        // 3. 计算 LQR 关键变量 Theta
       // Theta = 90度 - 腿相对于身体的角度 - 身体相对于地面的角度
       // 这代表了“轮腿连线”相对于“重力方向”的绝对倾角
 ...
 * @param  none
 */
 // 算速度 (BodyMotionObserve) —— 卡尔曼滤波
 // 编码器打滑时不准，且瞬时噪声极大  加速度计积分漂移，时间越长偏差越大。
 // 卡尔曼滤波器把这两个融合成一个可靠的数据。
static void BodyMotionObserve(void)
{
    // clang-format off
...
void ChassisReference(void)
{
    ...
    // {
    //     // 云台跟随模式下，控制量为云台坐标系下的速度，需要进行坐标转换
 ...
 * 平衡的大脑 —— LQR 控制器
 // 逻辑流程：
 // 查表(Gain Scheduling)：看看现在腿有多长，换一套对应的 K 参数。
 // 算误差：我歪了多少？速度快了多少？
 // 矩阵乘法：u = −K⋅x
 // 叠加转向：想转弯？左右轮子加减一点力。
 */
static void LocomotionController(void)
{
    ...
    for (uint8_t i = 0; i < 2; i++)
    {
        // [关键步骤 1] 变增益：根据腿长 L0 获取 K 矩阵
        // 因为腿伸长后，物理特性变了，必须换参数！
        GetK(CHASSIS.fdb.leg[i].rod.L0, k, is_take_off);

        // clang-format off
        // [关键步骤 2] 组装误差向量 x
        // 误差 = 实际观测值(fdb) - 期望目标值(ref)
        ...
        // 身体角速度误差
        x[5] = X5_OFFSET + (CHASSIS.fdb.leg_state[i].phi_dot - CHASSIS.ref.leg_state[i].phi_dot);        // clang-format on

        // [关键步骤 3] LQR 计算
        // 输出两个力矩：(有for循环，计算两条腿的力矩)
        // T_Tp[0]: 轮子力矩 (主要负责前后平衡和移动)
        // T_Tp[1]: 髋关节力矩 (辅助调节身体姿态)
        CalcLQR(k, x, T_Tp);
        // 保存结果
...
static void LegTorqueController(void)
{
    ...
    // 输入：
    //   1. rod.F  (刚才算的总推力)
    //   2. rod.Tp (LQR 算出来的髋关节力矩)
    //   3. J      (雅可比矩阵)
    // 输出：
    //   joint.T   (两个关节电机实际要输出的力矩)
    CalcVmc(
...
{
    // 1. 运行 LQR 算法
    // 产出：cmd.leg[i].wheel.T (轮子驱动力矩)
    // 产出：cmd.leg[i].rod.Tp  (髋关节平衡力矩)
    LocomotionController();
    // 2. 运行 VMC 算法
    // 产出：cmd.leg[i].joint.T (两个关节电机的最终力矩)
    LegTorqueController();
...
static void SendWheelMotorCmd(void)
{
    switch (CHASSIS.mode) {
        ...
        case CHASSIS_CALIBRATE: {
            DjiMultipleM3508TorqueControl(WHEEL_CAN, 0, 0, 0, 0);
        } break;
        case CHASSIS_OFF: {
            DjiMultipleM3508TorqueControl(WHEEL_CAN, 0, 0, 0, 0);
        } break;
